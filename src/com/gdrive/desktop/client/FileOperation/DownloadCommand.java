...
public class DownloadCommand extends ICommand {
	 ...
	 * file to download
	 */
	public File mFileToDownload;

	/**
	 * directory path
	 */
	public String mLocalDirPath;
	...
	protected int execute() throws Exception {
		...
		if (mFileToDownload.getDownloadUrl() != null
				&& mFileToDownload.getDownloadUrl().length() > 0) {
			try {
				// HttpResponse resp =
				// SharedInstances.DRIVE.getRequestFactory().buildGetRequest(new
				// GenericUrl(mFileToDownload.getDownloadUrl()))
				// .execute();
				// InputStream inputStream = resp.getContent();

				java.io.File file = new java.io.File(mLocalDirPath + "/"
						+ mFileToDownload.getTitle());
				...
				MediaHttpDownloader downloader = new MediaHttpDownloader(
						...
						DriveDesktopClient.DRIVE.getRequestFactory()
								.getInitializer());
				enableProgressListoner(downloader);
				downloader.download(
						new GenericUrl(mFileToDownload.getDownloadUrl()),
						fileoutputStream);
	...
	/**
	 * @param mProgressListoner
	 *            the mProgressListoner to set
	 */
